...
var import_drizzle_orm, import_pg_core, import_drizzle_zod, users, insertUserSchema, clashMessages, insertClashMessageSchema, tasks, insertTaskSchema, feedback, insertFeedbackSchema, groups, insertGroupSchema, groupMembers, insertGroupMemberSchema, notifications, insertNotificationSchema, appVersions, insertAppVersionSchema;
var init_schema = __esm({
  "shared/schema.ts"() {
    ...
    users = (0, import_pg_core.pgTable)("users", {
      ...
      googleId: (0, import_pg_core.text)("google_id").unique(),
      ...
      apiToken: (0, import_pg_core.text)("api_token").unique(),
      totalFocusTime: (0, import_pg_core.integer)("total_focus_time").default(0),
      // in minutes
      todayFocusTime: (0, import_pg_core.integer)("today_focus_time").default(0),
      // in minutes
      lastFocusDate: (0, import_pg_core.text)("last_focus_date"),
      // YYYY-MM-DD
      avatar: (0, import_pg_core.text)("avatar"),
      pushToken: (0, import_pg_core.text)("push_token"),
      clashChatNotifications: (0, import_pg_core.boolean)("clash_chat_notifications").default(true),
      isPro: (0, import_pg_core.boolean)("is_pro").default(false),
      proExpiresAt: (0, import_pg_core.text)("pro_expires_at"),
      // ISO timestamp
      stripeSubscriptionId: (0, import_pg_core.text)("stripe_subscription_id"),
      createdAt: (0, import_pg_core.text)("created_at").default(import_drizzle_orm.sql`now()`)
      // ISO timestamp for free trial
    });
    ...
    clashMessages = (0, import_pg_core.pgTable)("clash_messages", {
      id: (0, import_pg_core.varchar)("id").primaryKey().default(import_drizzle_orm.sql`gen_random_uuid()`),
      userId: (0, import_pg_core.varchar)("user_id").references(() => users.id),
      groupId: (0, import_pg_core.varchar)("group_id").references(() => groups.id),
      // Added groupId
      content: (0, import_pg_core.text)("content").notNull(),
      timestamp: (0, import_pg_core.text)("timestamp").default((/* @__PURE__ */ new Date()).toISOString())
    });
    insertClashMessageSchema = (0, import_drizzle_zod.createInsertSchema)(clashMessages).pick({
      userId: true,
      content: true
    });
    tasks = (0, import_pg_core.pgTable)("tasks", {
      ...
      courseCode: (0, import_pg_core.text)("course_code").notNull(),
      sectionId: (0, import_pg_core.text)("section_id").notNull(),
      ...
      notificationTime: (0, import_pg_core.integer)("notification_time").default(24 * 60)
      // Minutes before deadline, default 24h
    });
    insertTaskSchema = (0, import_drizzle_zod.createInsertSchema)(tasks).pick({
      type: true,
      title: true,
      courseCode: true,
      sectionId: true,
    ...
    feedback = (0, import_pg_core.pgTable)("feedback", {
      id: (0, import_pg_core.varchar)("id").primaryKey().default(import_drizzle_orm.sql`gen_random_uuid()`),
      userId: (0, import_pg_core.varchar)("user_id").references(() => users.id),
      content: (0, import_pg_core.text)("content").notNull(),
      createdAt: (0, import_pg_core.text)("created_at").default((/* @__PURE__ */ new Date()).toISOString())
    });
    insertFeedbackSchema = (0, import_drizzle_zod.createInsertSchema)(feedback).pick({
      userId: true,
      content: true
    });
    groups = (0, import_pg_core.pgTable)("groups", {
      id: (0, import_pg_core.varchar)("id").primaryKey().default(import_drizzle_orm.sql`gen_random_uuid()`),
    ...
    insertGroupSchema = (0, import_drizzle_zod.createInsertSchema)(groups).pick({
      name: true,
      code: true,
      createdBy: true
    });
    groupMembers = (0, import_pg_core.pgTable)("group_members", {
      id: (0, import_pg_core.varchar)("id").primaryKey().default(import_drizzle_orm.sql`gen_random_uuid()`),
      groupId: (0, import_pg_core.varchar)("group_id").references(() => groups.id),
      userId: (0, import_pg_core.varchar)("user_id").references(() => users.id),
      joinedAt: (0, import_pg_core.text)("joined_at").default((/* @__PURE__ */ new Date()).toISOString())
    });
    insertGroupMemberSchema = (0, import_drizzle_zod.createInsertSchema)(groupMembers).pick({
      groupId: true,
      userId: true
    });
    ...
    insertNotificationSchema = (0, import_drizzle_zod.createInsertSchema)(notifications).pick({
      userId: true,
      title: true,
      body: true,
      status: true
    });
    appVersions = (0, import_pg_core.pgTable)("app_versions", {
      id: (0, import_pg_core.varchar)("id").primaryKey().default(import_drizzle_orm.sql`gen_random_uuid()`),
      versionCode: (0, import_pg_core.integer)("version_code").notNull(),
      versionName: (0, import_pg_core.text)("version_name").notNull(),
      apkUrl: (0, import_pg_core.text)("apk_url").notNull(),
      releaseNotes: (0, import_pg_core.text)("release_notes"),
      createdAt: (0, import_pg_core.text)("created_at").default((/* @__PURE__ */ new Date()).toISOString())
    });
    insertAppVersionSchema = (0, import_drizzle_zod.createInsertSchema)(appVersions).pick({
      versionCode: true,
      versionName: true,
      apkUrl: true,
      releaseNotes: true
...
var init_db = __esm({
  "server/db.ts"() {
    ...
    init_schema();
    pool = process.env.DATABASE_URL ? new import_pg.Pool({
...
var import_express2, import_stripe, import_drizzle_orm3, router, stripeKey, stripe, PLANS, stripe_default;
var init_stripe = __esm({
  "server/stripe.ts"() {
    ...
    stripe = new import_stripe.default(stripeKey || "sk_test_placeholder", {
    ...
    router.get("/success", async (req, res) => {
      ...
      try {
        if (process.env.STRIPE_SECRET_KEY && sessionId) {
          ...
          const durationDays = parseInt(session2.metadata?.durationDays || "30");
          if (userId && session2.payment_status === "paid") {
...
var MemStorage = class {
  ...
  constructor() {
    ...
    const adminId = (0, import_crypto.randomUUID)();
  ...
  async createUser(insertUser) {
    ...
    const user = {
      ...
      lastActive: (/* @__PURE__ */ new Date()).toISOString(),
      apiToken: (0, import_crypto.randomUUID)(),
      totalFocusTime: 0,
      todayFocusTime: 0,
      lastFocusDate: null,
...
var DatabaseStorage = class {
  ...
  async getUserByGoogleId(googleId) {
    const [user] = await db.select().from(users).where((0, import_drizzle_orm2.eq)(users.googleId, googleId));
    return user;
  }
  async getUserByToken(token) {
    const [user] = await db.select().from(users).where((0, import_drizzle_orm2.eq)(users.apiToken, token));
    return user;
  }
  async createUser(insertUser) {
  ...
  async getTasks(userId) {
    if (userId) {
      return await db.select().from(tasks).where((0, import_drizzle_orm2.eq)(tasks.userId, userId));
    }
  ...
  async getNotifications() {
    const notifs = await db.select().from(notifications).orderBy((0, import_drizzle_orm2.desc)(notifications.createdAt));
    return notifs.map((n) => ({
      title: n.title,
      body: n.body,
  ...
  async deleteTask(id) {
    await db.delete(tasks).where((0, import_drizzle_orm2.eq)(tasks.id, id));
  }
  async createFeedback(insertFeedback) {
  ...
  async getAllFeedback() {
    return await db.select().from(feedback).orderBy((0, import_drizzle_orm2.desc)(feedback.createdAt));
  }
  async updateUserActivity(userId) {
    await db.update(users).set({ lastActive: (/* @__PURE__ */ new Date()).toISOString() }).where((0, import_drizzle_orm2.eq)(users.id, userId));
  }
  async updateUserRole(userId, role) {
    await db.update(users).set({ role }).where((0, import_drizzle_orm2.eq)(users.id, userId));
  }
  async updateUserStats(userId, totalTime, todayTime, lastDate) {
    await db.update(users).set({
      totalFocusTime: totalTime,
  ...
  async getLeaderboard() {
    return await db.select().from(users).orderBy((0, import_drizzle_orm2.desc)(users.totalFocusTime)).limit(50);
  }
  ...
  async getGroup(id) {
    const [group] = await db.select().from(groups).where((0, import_drizzle_orm2.eq)(groups.id, id));
    return group;
  }
  async getGroupByCode(code) {
    const [group] = await db.select().from(groups).where((0, import_drizzle_orm2.eq)(groups.code, code));
    return group;
  }
  async getUserGroups(userId) {
    const members = await db.select().from(groupMembers).where((0, import_drizzle_orm2.eq)(groupMembers.userId, userId));
    const userGroups = [];
    for (const member of members) {
      if (member.groupId) {
        const group = await this.getGroup(member.groupId);
        if (group) {
          const memberCount = await db.select({ count: groupMembers.id }).from(groupMembers).where((0, import_drizzle_orm2.eq)(groupMembers.groupId, group.id));
  ...
  async joinGroup(groupId, userId) {
    const [existing] = await db.select().from(groupMembers).where((0, import_drizzle_orm2.and)((0, import_drizzle_orm2.eq)(groupMembers.groupId, groupId), (0, import_drizzle_orm2.eq)(groupMembers.userId, userId)));
    if (!existing) {
  ...
  async removeGroupMember(groupId, userId) {
    await db.delete(groupMembers).where((0, import_drizzle_orm2.and)((0, import_drizzle_orm2.eq)(groupMembers.groupId, groupId), (0, import_drizzle_orm2.eq)(groupMembers.userId, userId)));
  }
  async getGroupMembers(groupId) {
    const members = await db.select().from(groupMembers).where((0, import_drizzle_orm2.eq)(groupMembers.groupId, groupId));
    const groupUsers = [];
    for (const member of members) {
      if (member.userId) {
  ...
  async deleteGroup(groupId) {
    await db.delete(groupMembers).where((0, import_drizzle_orm2.eq)(groupMembers.groupId, groupId));
    await db.delete(groups).where((0, import_drizzle_orm2.eq)(groups.id, groupId));
  }
  async updatePushToken(userId, token) {
    await db.update(users).set({ pushToken: token }).where((0, import_drizzle_orm2.eq)(users.id, userId));
  }
  async seed() {
    const adminEmail = "admin@assignflow.com";
    const [existing] = await db.select().from(users).where((0, import_drizzle_orm2.eq)(users.email, adminEmail));
    if (!existing) {
    ...
    const [sumitUser] = await db.select().from(users).where((0, import_drizzle_orm2.eq)(users.username, "sumitkumar"));
    if (!sumitUser) {
  ...
  async getLatestAppVersion() {
    const [version] = await db.select().from(appVersions).orderBy((0, import_drizzle_orm2.desc)(appVersions.versionCode)).limit(1);
    return version;
  }
  // Encryption helpers
  ...
  async getClashMessages(groupId) {
    ...
    }).from(clashMessages).leftJoin(users, (0, import_drizzle_orm2.eq)(clashMessages.userId, users.id)).where((0, import_drizzle_orm2.eq)(clashMessages.groupId, groupId)).orderBy(clashMessages.timestamp);
  ...
  async cleanupOldMessages() {
    const sevenDaysAgo = /* @__PURE__ */ new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
    await db.delete(clashMessages).where((0, import_drizzle_orm2.lt)(clashMessages.timestamp, sevenDaysAgo.toISOString()));
  }
  async toggleClashNotifications(userId, enabled) {
    await db.update(users).set({ clashChatNotifications: enabled }).where((0, import_drizzle_orm2.eq)(users.id, userId));
  }
};
...
var auth_default = import_passport.default;
...
function serveStatic(app2) {
  const distPath = import_path.default.resolve(process.cwd(), "public");
...
async function registerRoutes(app2) {
  ...
  app2.get("/api/updates/check", async (req, res) => {
    try {
      ...
      const baseUrl = `${protocol}://${host}`;
      const bundlePath = path2.resolve(process.cwd(), `updates/bundle-${latestVersion}.zip`);
      let size = 0;
      if (fs2.existsSync(bundlePath)) {
        size = fs2.statSync(bundlePath).size;
      }
      res.json({
        updateAvailable: true,
        latestVersion,
        currentVersion,
        updateUrl: `${baseUrl}/api/updates/download/${latestVersion}`,
        releaseNotes: "v1.0.33: Fixed social links & OTA system improvements.",
        critical: false,
  ...
  app2.get("/api/debug/ls-updates", async (req, res) => {
    try {
      const fs2 = await import("fs");
      const path2 = await import("path");
      const cwd = process.cwd();
      const updatesPath = path2.resolve(cwd, "updates");
      let updatesExists = fs2.existsSync(updatesPath);
      let files = [];
      res.json({
        cwd,
        updatesPath,
  ...
  app2.get("/api/updates/download/:version", async (req, res) => {
    try {
      ...
      console.log(`[OTA] Download request for: ${bundlePath}`);
      if (!fs2.existsSync(bundlePath)) {
        console.error(`[OTA] Bundle not found: ${bundlePath}`);
        return res.status(404).json({ message: "Update bundle not found" });
      }
      const fileBuffer = fs2.readFileSync(bundlePath);
      console.log(`[OTA] Sending bundle: ${bundlePath}, size: ${fileBuffer.length}`);
  ...
  app2.post("/api/register", async (req, res) => {
    try {
      ...
      try {
        let socialGroup = await storage.getGroupByCode("SOCIAL");
        if (!socialGroup) {
          socialGroup = await storage.createGroup("Social", user.id);
        }
        await storage.joinGroup(socialGroup.id, user.id);
        console.log(`User ${username} auto-joined Social group`);
      } catch (groupError) {
        console.error("Failed to auto-join Social group:", groupError);
      }
      req.login(user, (err) => {
...
var PostgresqlStore = (0, import_connect_pg_simple.default)(import_express_session.default);
...
var handler = async (event, context) => {
  ...
  if (!serverlessHandler) {
    try {
      await setupApp();
      serverlessHandler = (0, import_serverless_http.default)(app, {
